...
function strategy(space, network, provider, addresses, options, snapshot) {
    return __awaiter(this, void 0, void 0, function () {
        var blockTag, counters, userAddresses, calls, response, result;
                ...
                case 1:
                    ...
                    calls = function () {
                        ...
                        addresses.map(function (address, i) {
                            for (var j = 1; j <= parseFloat(units_1.formatUnits(counters[i].toString(), 0)); j++) {
                                userAddresses.push(address);
                                callsArr = __spreadArrays(callsArr, [
                    ...
                    return [4 /*yield*/, utils_1.multicall(network, provider, abi, calls(), {
                            blockTag: blockTag
                        })];
                case 2:
                    response = _a.sent();
                    result = response.reduce(function (acc, cur, i) {
                        if (acc[userAddresses[i]]) {
                            acc[userAddresses[i]] += parseFloat(units_1.formatUnits(cur.amount.toString(), options.decimals));
                        }
                        else {
                            acc[userAddresses[i]] = parseFloat(units_1.formatUnits(cur.amount.toString(), options.decimals));
                        }
